...
const ProductItemDetails = ({product}) => {
  ...
  return (
    <div className='grid grid-cols-1 md:grid-cols-2 p-7 bg-white text-black'>
      <Image src={imageUrl} width={300} height={300} className='p-5 h-[320px] w-[300px] object-contain rounded-lg'/>
      <div className='flex flex-col gap-3'>
        ...
        <div className='flex flex-col items-baseline gap-3'>
            ...
            <h2 className='text-2xl'>= ${(quantity *productTotalPrice).toFixed(2)}</h2>
            </div>
        ...
        <h2 className='pt-mt-10'><span className='font-bold'>Category:</span>{product.attributes.categories.data[0].attributes.Name}</h2>
      </div>
    </div>
  )
}
